...
public class BlueRightEverything extends LinearOpMode {
	...
	public void runOpMode() throws InterruptedException {
		...
		OpenGLMatrix targetPos = OpenGLMatrix.translation(-850, 660, 0)
		...
		if (hardware.isYellow()) {
			...
			hardware.driveForward(50, .5, 2_000, this);
			hardware.driveForward(-200, .5, 2_000, this);
			hardware.sampler.setPosition(1);
			hardware.driveForward(-1412, .5, 5_000, this);
		} else {
			...
			if (hardware.isYellow()) {
				...
				hardware.driveForward(-200, .5, 2_000, this);
				hardware.sampler.setPosition(1);
				hardware.driveForward(-981, .5, 3_000, this);
			} else {
				hardware.driveForward(-431, .5, 3_000, this);
				hardware.sampler.setPosition(.18);
				hardware.driveForward(50, .5, 2_000, this);
				hardware.driveForward(-200, .5, 2_000, this);
				hardware.sampler.setPosition(1);
				hardware.driveForward(-550, .5, 2_000, this);
			}
		}
		hardware.turnLeft(-45, 1, 2_000, this);
		hardware.driveForward(1300, 1, 5_000, this);
